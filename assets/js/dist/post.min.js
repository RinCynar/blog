  ...
  */
  ...
  function paletteFromSeed(hex, mode) {
    ...
    return {
      ...
      '--md-on-primary-container': dark ? hslToHex(h, 70, 92) : hslToHex(h, 70, 16),
      '--md-secondary': dark ? hslToHex(h + 18, 18, 80) : hslToHex(h + 18, 16, 38),
      '--md-secondary-container': dark ? hslToHex(h + 18, 16, 28) : hslToHex(h + 18, 40, 92),
      '--md-on-secondary-container': dark ? hslToHex(h + 18, 20, 92) : hslToHex(h + 18, 20, 16)
    };
  }
  function applyPalette(hex) {
    const tokens = paletteFromSeed(hex, currentMode$1());
    if (!tokens) return;
  ...
  function getFocusable(root) {
    if (!root) return [];
    return [...root.querySelectorAll('a[href], button:not([disabled]), textarea, input, select, [tabindex]:not([tabindex="-1"])')].filter(el => !el.hasAttribute('disabled') && el.getAttribute('aria-hidden') !== 'true' && isVisible(el));
  }
